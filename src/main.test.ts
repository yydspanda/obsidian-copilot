...
jest.mock("@/state/vaultDataAtoms", () => ({
  VaultDataManager: { startLifecycle: jest.fn(() => ({ cleanup: jest.fn() })) },
}));
jest.mock("@/knowledge/integration/KnowledgePluginIntegration", () => ({
  KnowledgePluginIntegration: class KnowledgePluginIntegration {},
}));
...
function createPluginUnderTest(calls: string[]) {
  ...
  Object.assign(plugin, {
    ...
    knowledgeIntegration: { close: jest.fn(() => calls.push("knowledge")) },
    vaultDataManager: { cleanup: jest.fn(() => calls.push("vaultData")) },
    ...
    describe("onunload()", () => {
      ...
      it("completes teardown when onload never assigned its collaborators", async () => {
        ...
        Object.assign(plugin, {
          ...
          knowledgeIntegration: undefined,
          vaultDataManager: undefined,
