...
describe("providerBaseUrl", () => {
  ...
  describe("isCatalogProviderDefaultEndpoint()", () => {
    it("https://github.com/yydspanda/obsidian-copilot/issues/3 recognizes official DeepSeek host and versioned endpoint spellings", () => {
      expect(isCatalogProviderDefaultEndpoint("deepseek", "https://api.deepseek.com")).toBe(true);
      expect(isCatalogProviderDefaultEndpoint("deepseek", "https://api.deepseek.com/v1/")).toBe(
        true
      );
    });

    it.each(["/v1beta", "/openai", "/openai/v1"])(
      "https://github.com/yydspanda/obsidian-copilot/issues/3 treats DeepSeek path override %s as custom",
      (path) => {
        expect(
          isCatalogProviderDefaultEndpoint("deepseek", `https://api.deepseek.com${path}`)
        ).toBe(false);
      }
    );
    ...
    it("recognizes the versioned Groq endpoint with a trailing slash", () => {
      expect(isCatalogProviderDefaultEndpoint("groq", "https://api.groq.com/openai/v1/")).toBe(
        true
      );
    });
