...
describe("vaultDataAtoms", () => {
  describe("VaultDataManager", () => {
    ...
    describe("initialize()", () => {
      it("reads tags through the App supplied to the manager", () => {
        const owner = createOwner();
        ...
        expect(getTagsFromNote).toHaveBeenCalledWith(owner.app, file, true);
        expect(getTagsFromNote).toHaveBeenCalledWith(owner.app, file, false);
      });
    });
  });
});
