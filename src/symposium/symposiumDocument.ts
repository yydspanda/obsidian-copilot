...
const IMAGE_DATA_URL_MEDIA_TYPE = /^data:image\/[a-z0-9.+-]+$/i;
const IMAGE_DATA_URL_PARAMETER = /^[a-z0-9=.+-]+$/i;
const IMAGE_DATA_URL_PAYLOAD = /^[a-z0-9+/=\s]+$/i;
const IMAGE_DATA_URL_VALIDATION_CHUNK_LENGTH = 64 * 1024;
...
function isEmbeddedImageSource(source: string): boolean {
  ...
  const [mediaType, ...parameters] = source.slice(0, headerEnd).split(";");
  const encoding = parameters.pop();
  if (
    !IMAGE_DATA_URL_MEDIA_TYPE.test(mediaType) ||
    encoding?.length !== "base64".length ||
    encoding.toLowerCase() !== "base64" ||
    parameters.some((parameter) => !IMAGE_DATA_URL_PARAMETER.test(parameter))
  ) {
    return false;
  }

  // Bounded chunks prevent V8's RegExp engine from exhausting its stack on near-limit payloads.
  for (
    let offset = headerEnd + 1;
    offset < source.length;
    offset += IMAGE_DATA_URL_VALIDATION_CHUNK_LENGTH
  ) {
    const chunk = source.slice(offset, offset + IMAGE_DATA_URL_VALIDATION_CHUNK_LENGTH);
    if (!IMAGE_DATA_URL_PAYLOAD.test(chunk)) {
      return false;
    }
  }
  return true;
