...
export class AgentSessionManager {
  ...
  async createSession(
    ...
    const requestedSeed = seedSelection ?? this.getSeedSelection(resolvedId) ?? undefined;
    const resolvedSeed =
  ...
  /**
   * Read the user's sticky model preference, normalizing compatible aliases.
   *
  ...
  getSeedSelection(backendId: BackendId): ModelSelection | null {
    ...
    const descriptor = this.opts.resolveDescriptor(backendId);
    const settings = getSettings();
    // A policy-rejected model must reach the session's refusal boundary, not
    // become a usable replacement merely because the catalog no longer lists it.
    // https://github.com/yydspanda/obsidian-copilot/issues/3
    if (descriptor?.normalizeSelection?.(saved, settings) === null) return saved;

    const offered = descriptor?.getEnabledModelEntries?.(settings);
