...
function ReviewStep({
  ...
  const runUpload = async (packed: PreparedReport) => {
    ...
    try {
      ...
      if (!mountedRef.current) return;
      if (outcome.ok) {
        // The page turn comes first, and not for style: the report is already
        // on the server, so a host that throws on the way to the browser must
        // not leave the flow stuck on "Uploading…" with the link unreachable.
        // No success state is set — `onUploaded` unmounts this step.
        onUploaded(outcome);
