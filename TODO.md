...
- 当前 Windows 启动只初始化插件私有 `knowledge-runtime-v1.json`（文件名稳定、outer schema v3）与 unavailable notice；Queue/Review/Manifest/Transaction/observation facades、文件 writer、compiler、startup recovery 和 query coordinator 尚未连到 Studio，因此不会展示伪造任务、调用模型或写 Wiki。
