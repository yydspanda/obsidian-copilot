...
- [x] 实现只读 production-facing snapshot loader Core、exact schema bridge、显式 allowlist profile source 与 Knowledge 专用 UTF-8 byte parser，定义并测试 Manifest facade、exact reader、Projects/settings generation 的可注入边界；这些端口尚未在插件中完成具体 production 组装。不传完整 provider settings，也不沿用会二次按路径读取或走闭源缓存的 legacy `FileParserManager`。
- [x] 实现 Knowledge 专用 provider execution binding、model adapter、确定性 prompt encoder 与首个 DeepSeek private transport：module-private Queue/Runtime/observation capability 重新绑定 source identity，不信任 `unbound_read_only` marker 或同型 job DTO；私有 route/credential closure 匹配 endpoint/routing/model configuration，DeepSeek concrete route 再绑定完整 pipeline profile。禁止回读 settings、复用 ChatModelManager/LangChain fallback、`safeFetch/requestUrl` 或依赖可变 provider defaults。workflow plan 在 source/parser/model 边界稳定复读 Manifest、schema 与 Projects/settings profile，Runtime proof 来自同一 atomic envelope；exact execution owner 与 per-compile model session 阻断跨 Runtime/plan/compiler 阶段拼接。production workflow generation 仍须覆盖 App/Vault/parser/settings/key/native-fetch owner 生命周期，并把 fresh adapter 在首次调用前直接交给预期 Compiler；真实 worker 接线前还须决定 transport typed error 到 Queue `rate_limit` 的脱敏映射。
- [ ] 在插件级 workflow generation 中完成 listener-before-crawl、allocated/bound observation recovery、跨重启 destructive-change authority/revalidation、全量 scan、startup Gate/release 与 worker 顺序接线；真实 executor 必须把 `readExpected` 返回的同一 bytes 直接交给 byte-only parser，并在 fresh model adapter 可被其他代码观察前完成预期 Compiler 组装。若 production 架构不能保证该私有交接，先实现 composer-issued pre-bind owner。完成前 Studio 继续 unavailable。
- [ ] DeepSeek production 接线前单独更新/迁移插件 model catalog 与 Knowledge-effective 配置：截至 2026-08-02 官方只接受 `deepseek-v4-flash` / `deepseek-v4-pro`，旧 `deepseek-chat` / `deepseek-reasoner` 已退役；thinking 模式不支持 `temperature/top_p`，而插件当前默认是 `temperature=0.1`、`reasoningEffort=low`。不得由 Knowledge transport 静默 alias/fallback/忽略参数；迁移必须显式选择 V4，并令 thinking profile 可审计地使用占位 `temperature=0` 且无 `topP`（wire 不发送二者），或显式关闭 thinking 后再应用采样参数；还须清理或显式拒绝 `frequencyPenalty`、`numCtx`、Responses API、prompt caching 与 provider routing 等未映射 generic 字段，同时回归普通 Chat 行为。
...
- [ ] 在现有前置 Barrier 后组装完整 recovery validator/file adapter、逐 Bundle startup Gate + conditional release、worker 隔离与恢复 UI；不能把保留的 user/rate-limit pause、empty/running Queue 当作“已完成 Gate”的持久证明，不得自动 continue 或把 `observed_clear` 当作 claim authority。
...
- 当前 Windows 启动初始化插件私有 `knowledge-runtime-v1.json`（文件名稳定、outer schema v3），再由 layout-ready Barrier 等待普通 Projects readiness、严格验证全部 project-owned Bundle，并通过稳定 port + 动态 session 发布 unavailable 状态。它刻意不运行 Queue/Review/Manifest/Transaction/observation facades、文件 writer、compiler、startup Gate/release 或 query coordinator，因此不会展示伪造任务、调用模型或写 Wiki。
