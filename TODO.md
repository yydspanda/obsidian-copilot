...
- `ProjectRegister` 已解决重复监听、失败残留与 unload 后复活；但既有 `ProjectFileManager`/`ProjectManager` singleton 的跨插件实例/跨 Vault app ownership，以及 cleanup 无法取消 `loadAllProjects()` 最终全局 cache commit，仍须在任何 ready Studio delegate 或跨 Vault knowledge ownership 前用 reset/rebind + lifecycle-owned commit token 解决。
