...
- [ ] 实现真实 Manifest repository、exact schema reader 与 Knowledge 专用 parser/provider/profile adapter，把当前 builder 的完整输入从插件配置和持久状态装配出来；不能把包含 API key 的完整 provider settings 传给 profile，也不能沿用会二次按路径读取或走闭源缓存的 legacy `FileParserManager`。
