...
- [ ] 实现 Knowledge 专用 provider execution binding 与 model adapter：用 module-private durable Queue/observation capability 重新绑定 source identity，不能信任可伪造的 `unbound_read_only` marker 或同型 job DTO；私有绑定 exact route/credential capability 并复证 endpoint/routing/profile 摘要，只消费 projection-only 行为，禁止回读可变 settings、复用 ChatModelManager fallback 或依赖未指纹的 provider defaults。workflow generation 必须覆盖 Runtime/Manifest、schema、Projects/settings、App/Vault 与 parser owner 的全部变化；否则在 Compiler/model 前增加原子 revision 或稳定多读证明。完成前当前 preparation 不得调用 Compiler/model。
- [ ] 在插件级 workflow generation 中完成 listener-before-crawl、allocated/bound observation recovery、跨重启 destructive-change authority/revalidation、全量 scan、startup Gate/release 与 worker 顺序接线；真实 executor 必须把 `readExpected` 返回的同一 bytes 直接交给 byte-only parser。完成前 Studio 继续 unavailable。
...
- production profile source 只能向 Pipeline fingerprint 传入显式 allowlisted model behavior 与受检静态 JSON 配置；Core 另防御性拒绝常见 credential-like 字段，但不把黑名单当成 allowlist 的替代品。未来 provider adapter 必须消费该 projection 和私有 route/credential binding，禁止再次读取完整 settings。
- Queue 的 `inputRevision` 必须由 source adapter 按 source 严格单调分配、跨重启保存，并在相同内容观察时同样推进；不得使用内存计数器或文件 mtime 代替。
- source adapter 必须在 watcher handler 开始、任何异步读取/解析之前生成全局永久唯一且跨重试稳定的 `captureId`（生产 adapter 使用随机 UUID/等价 opaque id）并分配 opaque observation token/`inputRevision`，再以 first-write-wins hand-off 绑定随后实际读到的 bytes hash 与 pipeline fingerprint。Queue adapter 只能凭该 token 原子消费 exact bound record；相同 capture 重放不增 revision且已 consumed 时不再调用 Queue，不同 payload 冲突，较新未绑定 allocation 不会丢弃仍可用的较低 bound read，较新 bound read 才会淘汰较低未终结观察。独立 Obsidian watcher、opaque plan Core、双采集 snapshot loader 与 UTF-8 byte parser 已实现 exact-byte/hash 身份边界；durable Queue job binding、重启协调、provider/compiler worker 和 plugin workflow 接线仍待完成。
