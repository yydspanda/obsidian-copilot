...
- [x] 实现 Knowledge 专用 provider execution binding 与 model adapter Core：module-private Queue/Runtime/observation capability 重新绑定 source identity，不信任 `unbound_read_only` marker 或同型 job DTO；私有 route/credential closure 匹配 endpoint/routing/full profile 摘要，只消费 projection-only 行为，禁止回读 settings、复用 ChatModelManager fallback 或依赖未指纹的 provider defaults。workflow plan 在 source/parser/model 边界稳定复读 Manifest、schema 与 Projects/settings profile，Runtime proof 则来自同一 atomic envelope；exact execution owner 与 per-compile model session 阻断跨 Runtime/plan/compiler 阶段拼接。production workflow generation 仍须在组装时覆盖 App/Vault/parser owner 生命周期，并把 fresh adapter 作为首次调用前的可信 capability 直接交给预期 Compiler。真实 provider transport、provider-side 响应 byte/token 限制与 prompt 编码仍是后续 adapter 工作。
