...
- [x] 完成 G.2j 独立 Vault source watcher adapter：严格 immutable registered-source watch plan、exact App/Vault/adapter owner、同步 capture authority/id、durable allocate-before-read、captured-path exact binary SHA-256、handoff-only commit、注入端口 success payload 复证、同 capture/token/payload 有界重试、跨 lifecycle/watch-plan continuation fail-close与同路径多 Bundle 隔离；rename/delete/folder 破坏事件同步让旧任务失权并 quarantine 对应 source，只有显式替换 watch plan 才能恢复。配套 exact-byte reader 在未来 executor 的 parser/model 前复证 Queue job hash，并返回 parser 必须消费的同一 bytes。真实 watch-plan discovery、recovery、startup/worker composition 仍保持 unavailable。
...
- [x] 实现独立 `ObsidianVaultSourceWatcher` production adapter：只接受已注册 source + immutable pipeline fingerprint 的严格 watch plan，绑定 exact App/Vault/adapter，先同步捕获 path/authority/captureId，再 durable allocate，随后用 owning Vault `DataAdapter.readBinary` 读取 exact bytes、计算 raw SHA-256并仅经 `SourceObservationHandoff` commit；watcher 会复证 reader/allocation/settlement success payload，相同 capture/token/hash 的 allocate/read/commit 重试不改 payload，旧 lifecycle/watch-plan continuation 不能继续 read/commit/publish。同路径多 Bundle 独立观察；rename/delete/folder 事件不伪造空内容，会同步隔离对应 source 并阻断同路径 ABA，直到显式替换 watch plan。`ObsidianExactSourceArtifactReader.readExpected` 为真实 executor 提供 parser/model 前的 job hash 复证与同一 bytes hand-off。该 adapter 尚未接入 `main.ts`，不会后台运行或开放 Studio。
- [ ] 从 strict Bundle config + durable Manifest + parser/provider/schema 配置构建真实 watch plan，并在插件级 workflow generation 中完成 listener-before-crawl、allocated/bound observation recovery、跨重启 destructive-change authority/revalidation、全量 scan、startup Gate/release 与 worker 顺序接线；真实 executor 必须把 `readExpected` 返回的同一 bytes 直接交给 parser。完成前 Studio 继续 unavailable。
