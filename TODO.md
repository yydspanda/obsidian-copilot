...
- [ ] 将真实 Windows source watcher 接到 `SourceObservationHandoff`，由 adapter 在任何 `await` 前生成稳定 `captureId`，从实际读取 bytes 计算 hash，并在 compiler 真正读取 artifact 时再次核对 job hash；接线前 Studio 继续 unavailable。
