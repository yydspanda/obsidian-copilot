...
Startup reconciliation for persisted pending and rejected reviews now exists as an isolated safety core, but it is not connected to plugin startup yet. Accepted reviews are never applied automatically during startup; their identity is classified against Queue, transaction, commit-ledger, and abandonment evidence from one atomic runtime snapshot. If an accepted apply claim exists but no journal was ever published, the core requires an explicit **Continue** or **Abandon** decision. Continue reloads the accepted content and rechecks the current Manifest before the transaction may inspect Wiki files. Abandon is allowed only when the same atomic update proves that no active journal, commit marker, or source-input success ledger exists; it keeps the accepted review as immutable history. A stale Manifest blocks Continue but does not fabricate evidence that Wiki writes began. These actions are tested but do not appear in Studio until the startup and UI adapters are connected.
...
The next implementation steps are plugin workflow wiring for the implemented recovery cores, a Studio recovery screen, durable `no_changes` success semantics, durable authorization for any future auto-apply policy, the remaining schema/link/source-artifact read-set, coordinated history retention, real parser/provider/compiler adapters, Chat's **Add to Knowledge** entry, grounded query/citation navigation, and end-to-end testing in a dedicated Windows Vault. The Studio should not be treated as a finished ingestion workflow until those integrations and tests are complete.
