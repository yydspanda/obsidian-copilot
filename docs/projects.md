...
This field does not change normal Project chat behavior. With one valid Bundle and a successful recovery/observation release, the plugin can ingest authorized sources in the background, run the two-stage DeepSeek compiler, and persist a proposal in durable Review. Preflight and the startup Gates are zero-network, but released compilation sends authorized knowledge content to DeepSeek and may incur provider charges. The background worker stops at `awaiting_review` and has no Apply authority. In the live Studio, a user can inspect Activity, reject a proposal, or explicitly accept eligible create/update selections; that separate command path revalidates durable state, runs the recoverable transaction, and finalizes its Manifest, Queue, journal, and ledger records. On Windows, dragging a Vault `.md`, `.markdown`, `.txt`, or `.pdf` file into Chat shows **Use in this chat** and **Add to Knowledge** before either state changes. Durable registration is offered only when one Bundle with one source root is live and the file already belongs to that root; it does not attach the file to Chat context. The Query tab searches only SHA-256-verified pages from the Bundle's accepted and committed Wiki and never falls back to other Vault notes. When exact Markdown source evidence can be re-proved, Query sends the question, bounded Wiki context, and those source excerpts to a separate DeepSeek answer route in one request, then validates every returned evidence ID before showing a read-only answer. With no eligible source evidence, it returns **Insufficient evidence** without calling the model. PDF sources are parsed locally from the already-verified raw bytes into bounded 1-based page evidence without Miyo, Brevilabs, converted Markdown caches, or OCR. An accepted PDF citation chip can open a positive validated page only after the exact `.pdf` raw-byte SHA-256 is re-proved; PDF citations can appear on retrieval hits, while direct PDF evidence for the H.2 synthesized-answer route remains disabled. A current Supported or Partial answer with at least one validated claim can be saved through Review. Save re-proves its answer and citations, creates a deterministic content-addressed managed Markdown source beneath the Bundle's one configured source root, and registers an exact `managed_copy` Manifest origin. The real watcher and Queue then compile it with `operation: query_writeback`; the first applied provenance points to that capture, whose content embeds the original evidence chain. Save never writes Wiki directly or fabricates a queued result, and the proposal still requires explicit Review and Apply. A nested create target requires its parent Wiki folder to exist on the Windows adapter; otherwise Review submission blocks before durable acceptance and does not create an untracked directory. Exact retry can converge an uncertain create/registration result, although a crash after create and before registration can leave an unregistered orphan capture. Nothing auto-accepts or auto-applies; delete remains disabled. Fresh create/update Apply, fail-closed external-edit handling, limited Recovery, scoped retrieval, exact Markdown quote navigation, H.2 grounded answers, the complete Save-to-Wiki capture-through-Apply chain, and Vault-source Chat capture have passed bounded acceptance in independent Windows test Vaults. A fresh PDF run also completed Vault registration, local production ingestion, two-stage DeepSeek compilation, explicit Review/Apply, accepted page provenance, and exact first/middle/last-page navigation in the main window and a popout. Source-byte drift, query replacement, and plugin reload revoked old PDF references without an unsafe open, and a new post-reload reference remained usable. The H.2 run covered real DeepSeek answers, source-fact and inference labels, all three answer statuses, exact citation selection, deterministic zero-request no-evidence behavior, reload cancellation, and query-local zero writes; direct PDF evidence remains disabled for that synthesized-answer route. Broader filesystem races and crash or power-loss behavior remain separate gates. Changing Settings or Project configuration automatically closes the old generation and starts a fresh validation generation while the plugin remains active; a plugin reload is not required for this revalidation. See [Personal Knowledge Studio](personal-knowledge.md) for the full availability and safety model.

For applied Wiki pages, the Manifest keeps the last true **Source Apply** body as the source baseline while an exact active **Forward revision** may become the effective current head. The inspector discloses the current origin and Source-evidence scope; the system retains and validates both hashes internally. Query reads the exact effective bytes without asking a model to repair or recompile them. Forward text still carries only its original Source citations, which do not become new proof for human-authored wording. Known outputs deduplicate exact bodies while retaining Source, Forward, or mixed origins and a per-row proposal reason. Forward-only historical rows cannot seed another proposal yet; mixed rows can do so only when their canonical detail authority is Source Apply.

Repeated Forward revisions form one exact live-head chain per Bundle/source/page. Each successor compares against the previous effective hash while preserving the immutable Source-applied baseline. A successful ordinary Source Apply supersedes that Forward layer only when it commits the same source's exact target page; unrelated-page applies and truthful `no_changes` results preserve it. An accepted Forward that has not started a journal can be ended with **End without writing**, which writes no Wiki, journal, ledger, or overlay. Sticky Forward recovery always rechecks exact bytes first and retries an exact change only from an eligible exact-before state. **Keep current (no write)** never mutates the Wiki; when zero-write terminalization is allowed, the durable outcome truthfully distinguishes abandonment before write, an uncertain write followed by external supersession, and a committed write followed by external supersession according to the journal phase. It never force-overwrites or labels every phase as external supersession. The Forward `recovery_required` card disables both commands while one is pending or the generation lacks capability, then reloads durable truth after each result. The newer effective-head, repeated-Forward, and terminal-recovery behaviors have automated and Windows path/case-key coverage, but the user's planned real Windows Obsidian acceptance run is still pending.

Knowledge Studio's **Import folder** action is the external-material entry point for a Bundle. It takes one explicit folder selection, copies only `.md`, `.markdown`, `.txt`, and `.pdf` files below `<source root>/<selected root name>/`, preserves nested paths, and registers each Vault copy as `managed_copy`. The external originals are read-only and their absolute path is not stored. Existing destinations with identical bytes are reused; conflicting bytes are reported and never overwritten. The sequential batch may complete partially and can be retried, while all generated changes continue through Activity, explicit Review, and explicit Apply. URL/browser-clipper and one-off Windows Explorer file ingestion are intentionally not separate Knowledge entry points. The production stable-port path has passed a bounded real-folder Windows run, including exact retry, missing-copy repair, unchanged external originals, completed Activity, and a clean console. The native system picker was not physically clicked in that run, and the compiler returned `no_changes`, so a picker interaction and a non-empty proposal taken through explicit Review and Apply remain separate acceptance work.

If the compiler proves that no Wiki page needs to change, the Runtime creates no empty Review. It atomically completes the exact Queue attempt and records a source- and pipeline-bound no-change marker while preserving accepted provenance. Runtime version 4 also returns an older unproved completion to pending recovery work instead of reusing it as success. A later exact-input observation skips another model call only when the latest Runtime Apply or no-change proof matches and every expected output is still an exact file with the recorded SHA-256. A missing file, directory, or byte change schedules a repair compile or retained rerun. A repair with file changes still stops at Review and never overwrites or applies the Wiki automatically. If the repair claims `no_changes` while output remains stale, it becomes a non-retryable unresolved repair without a success marker; the same input cannot start another automatic model run until an up-to-date observation proves restoration, while genuinely newer source or pipeline work remains eligible. Events for a tracked output re-observe its original source owners, including every owner of a shared page, while uncertain authority or I/O fails closed without changing Queue state. Output checks are deferred during Apply and repeated after the required generation refresh. The intact-output path has passed a bounded Windows run: a real two-stage DeepSeek compile produced a no-change marker without Review or Wiki writes, and both a plugin reload and a disable/enable lifecycle advanced the authoritative observation with zero model requests and no durable success-state changes. A dedicated Windows missing-output run also passed: intact cold/reload scans made zero model requests, removing one accepted output produced one Review-gated two-stage DeepSeek repair, Reject did not write the Wiki, and exact restoration returned to a clean running state with zero further model requests and the exact output bytes restored. Byte mismatch, directory, shared-owner, Apply-interleaving, crash-recovery, adversarial no-change repair, and resource-limit cases remain automated evidence rather than a claimed Windows byte-change run.
