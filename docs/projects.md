...
If the compiler proves that no Wiki page needs to change, the Runtime creates no empty Review. It atomically completes the exact Queue attempt and records a source- and pipeline-bound no-change marker while preserving accepted provenance. Runtime version 4 also returns an older unproved completion to pending recovery work instead of reusing it as success. A later exact-input observation can skip another model call while the expected generated outputs remain intact; production repair for a missing or byte-changed generated output remains a separate pending gate. This path has passed a bounded Windows run: a real two-stage DeepSeek compile produced a no-change marker without Review or Wiki writes, and both a plugin reload and a disable/enable lifecycle advanced the authoritative observation with zero model requests and no durable success-state changes.
