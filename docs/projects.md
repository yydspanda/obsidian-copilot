...
## Personal Knowledge Studio Bundle (advanced, Windows only)

This branch preserves an optional `copilot-project-knowledge-bundle` object in a Project's `project.md` frontmatter. It defines the source, generated Wiki, and schema boundaries owned by Personal Knowledge Studio:
...
This field does not change normal Project chat behavior. Knowledge startup validates every configured Project together and fails closed on unknown fields, malformed Vault-relative paths, duplicate Bundle IDs, or case-insensitive Windows overlap between source, Wiki, and schema boundaries. It never rewrites an unsafe path. The current Studio admits exactly one valid Bundle; it does not choose implicitly when several exist.

After startup and recovery checks succeed, the Knowledge worker may send authorized source, schema, and existing target content to the configured DeepSeek route and incur provider charges. It can create durable Activity and Review records, but it cannot accept or apply a proposal. Wiki create or update requires the user's explicit **Review** and **Apply**; delete, auto-accept, auto-apply, and force-overwrite remain disabled.

Query searches only SHA-256-verified pages from the Bundle's accepted Wiki and re-proves eligible source evidence before showing citations. PDF sources use local, byte-verified page extraction without OCR; direct PDF evidence remains unavailable to the synthesized-answer route. **Save to Wiki** creates a managed source and returns through the same compiler, Review, and Apply pipeline instead of writing the Wiki directly.

The Manifest preserves the last true **Source Apply** hash while an exact **Forward revision** may become the effective current head. The inspector shows that origin and the Source-evidence scope; both hashes are retained and validated internally. Repeated Forward revisions form one exact chain. An unrelated Source Apply or truthful `no_changes` result preserves that head, while a successful Apply from the same source to the same page supersedes it. **End without writing** and **Keep current (no write)** never mutate Wiki bytes; recovery rechecks exact bytes before retrying an eligible compare-and-swap.

Automated coverage and bounded Windows runs cover the established workflow, while the newer effective-head, repeated-Forward, and terminal-recovery behavior is still awaiting the user's planned real Windows Obsidian acceptance. See [Personal Knowledge Studio](personal-knowledge.md) for the complete safety, recovery, import, freshness, and availability model, or follow the [中文使用手册](knowledge/zh-CN/index.md).
