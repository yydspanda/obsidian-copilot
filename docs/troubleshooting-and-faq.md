...
**Problem**: A saved DeepSeek model is disabled, or a V4 request is rejected before it is sent

**Fix**:

- Select `deepseek-v4-flash` or `deepseek-v4-pro` explicitly. The retired `deepseek-chat` and `deepseek-reasoner` identities are not redirected automatically.
- Use Minimal effort when you want Temperature or Top P sampling controls.
- With High or XHigh thinking, remove sampling overrides; Copilot omits Temperature and Top P from the request. Low and Medium are not offered because DeepSeek maps both to High.
- Frequency Penalty is not supported for direct DeepSeek models.
- DeepSeek thinking responses can be long; increase Max Tokens if a valid response ends too early.
