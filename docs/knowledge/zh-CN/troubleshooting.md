...
如果普通提案显示 `Apply paused`，表示 Bundle 当前不允许运行 Apply，并非没有选中。可以继续勾选、编辑或整项拒绝，但不能提交接受项。先在 Activity 检查暂停原因；不要直接 Resume 来解锁按钮，因为这也可能恢复排队任务并调用模型。暂停检查若在保存接受决定前拦截提交，提案仍待审核，不需要因此进行恢复。

### 提交按钮显示 Proposal out of date

旧提案使用的 Knowledge 配置与当前配置不一致，或它的来源已不在当前配置中。即使文件卡显示 `current`，也只代表目标文件符合预期，不能据此应用旧提案。重复 Apply 或 Resume 不会把它变成兼容的新提案。

可以继续查看；确认放弃时，先选 `Skip all changes`，再点 `Reject proposal`。系统不会自动改写、删除或重新生成旧提案。先检查 Activity：正在运行的 Bundle 在结束旧提案后，可能开始处理已保留的新任务并调用模型；暂停的 Bundle 不会被自动恢复。新的处理结果仍须重新审核。

如果提交后提示无法验证当前配置或来源，而不是确定的配置不匹配，先检查来源、配置和读取状态，再刷新；这不等于已证明文件内容发生变化。不要编辑内部记录或哈希来强制解锁。
...
单个来源的原始文件最多 8 MiB。启动扫描、来源重新观察和执行任务时，都会先检查当前文件大小；读取后还会再次检查返回的字节数，超限内容不会继续复制、计算哈希或送去解析。这个限制同样适用于已经登记过的文本和 PDF，不会自动截断文件。

请先缩减或拆分过大的来源，再重新加载插件检查。仅把文件改名或换扩展名不能绕过大小限制。
