...
普通提案仍可查看、勾选或编辑，选择只保存在当前会话；重新加载插件会清空这些未提交选择。顶部会说明 Apply 已暂停，含接受项的最终按钮显示 `Apply paused`，不能提交。全部跳过后，仍可用 `Reject proposal` 明确拒绝整个提案，这不会写 Wiki，也不会恢复后台任务。

若暂停恰好发生在提交检查期间，系统会在保存接受决定前再次检查暂停状态；被暂停拦截的提案仍待审核，不会仅因这次拦截进入恢复流程。已经持久接受之后发生的中断，仍按原有恢复流程处理。

需要继续 Apply 时，先去 Activity 了解暂停原因和排队任务。不要为了点亮按钮盲目点击 `Resume bundle`：恢复会允许排队任务继续处理，可能调用模型；Review 不会替你自动恢复。

### 旧提案与当前配置不一致时

升级或调整 Knowledge 配置后，旧提案可能不再适用。顶部会显示过期说明，含接受项的提交按钮显示 `Proposal out of date`，不能 Apply。这是按配置身份核对，不是按提案保存了多久判断。文件显示 `current` 只表示目标文件没有变化，不代表旧提案仍适合当前配置。

旧提案会保留，仍可查看和准备选择，但不会自动修改成新版本，也不会自动拒绝或重新生成。确认不再需要时，可先选 `Skip all changes`，再点 `Reject proposal` 明确结束它；拒绝本身不会写 Wiki。如果 Bundle 正在运行，结束旧提案可能释放已排队的重新处理任务并调用模型，操作前先检查 Activity。后续产生的新提案仍需重新审核。

如果配置在打开页面后才变化，提交时也会重新核验。无法验证当前配置或来源的旧提案会被阻止接受并给出说明，不会仅因此创建恢复任务。正常提案仍沿用原有审核和 Apply 流程。
