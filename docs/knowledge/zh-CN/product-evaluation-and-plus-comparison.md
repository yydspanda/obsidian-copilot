...
本次复评不是把旧分数重新包装。以下变化已经实装并获得自动化或有界 Windows 证据：
...
- **发布门禁提高。** 当前完整记录为 262 个 Jest suite、4,387 个测试通过，并通过 TypeScript、格式、ESLint、生产构建、差异和敏感信息检查。

这些变化把核心闭环从“工程上能连起来”推进到“已配置用户可以按流程使用”。但它们没有消除首次配置、Review 修正、Wiki 回滚和长期复用验证的缺口。
...
评分是产品判断，不是实验室基准。Knowledge Studio 的判断来自当前代码、测试、手册、Windows 有界验收和本轮真实用户反馈；Plus 的判断只来自官方公开材料，未独立审计其专有服务端。
...
| 工程安全与可靠性 | **8.8/10** | 262 suites / 4,387 tests，加上多轮 Windows 有界验收，证明安全边界受到认真保护；它仍不等于所有设备和长期场景无风险。    |
| 核心闭环完整度   | **7.6/10** | Chat Draft 和来源生命周期补上了两处关键断点；Review 修订、Wiki 回滚和完整 Golden Flow 仍未闭合。                       |
| 首次配置体验     |     3.5/10 | Project、Bundle、三个路径、Schema、Chat 模型、Knowledge 模型和 API Key 同时暴露，仍像工程控制台。                      |
...
| 模型配置与网络是独立故障域 | 本轮 Chat Draft 验收复用了已保存 AI 回复；当前选中的 OpenRouter Gemini 配置缺 API Key，尚未证明一次新的 Chat 生成 | 增加模型 readiness 检查，把“Chat 模型不可用”和“Knowledge Studio 不可用”分开提示。 |
...
| 用户旅程                |   评分 | 主要感受                                                                         |
| ----------------------- | -----: | -------------------------------------------------------------------------------- |
| 第一次配置              | 3.5/10 | 概念和模型配置过多，不知道哪个是必须的。                                         |
...
最核心的 UX 矛盾是：**工程信任约 8.8/10，界面自解释约 5/10。** 用户愿意学完之后能顺畅使用；第一次打开的人仍容易在 Project、Bundle、Manuals、Sources、Wiki、Schema、Chat 模型和 Knowledge 模型之间迷路。
...
- Wiki 版本、差异、撤销和受控修订；
- 首配向导、安全默认 Bundle/Schema、Chat/Knowledge 模型 readiness；
...
1. 首配向导、安全默认值与模型 readiness 检查；
2. Review 就地编辑、打开证据、反馈重生成；
3. Wiki 版本/差异/撤销；
4. Studio 内统一 Source 入口，并解释文件夹快照、Vault 文件和 Chat Draft 的差别；
5. Knowledge Draft 显式关联原始材料；
