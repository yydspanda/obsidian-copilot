...
新的分析校验失败会在原错误说明后保留最多五个安全的检查编号，不包含模型回复或笔记正文。例如：

- `compiler_claim_ungrounded`：模型给出的结论缺少支持它的来源引用。
- `compiler_target_manifest_authorization_missing`：模型想修改一个已经受管理的页面，但当前来源没有修改它的权限。检查 Rules 是否要求多份材料强行写进同一页；不要删除历史或放宽权限来绕过。

旧失败记录没有保存这些细节，升级不会补出当时的原因。历史条数多不等于当前仍在执行；重启后重新观察未成功的来源可能产生新任务，因此不应靠反复重启解决分析错误。
