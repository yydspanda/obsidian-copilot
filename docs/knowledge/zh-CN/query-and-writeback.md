...
保存期间不用重复点击；此时点击引用也不会打断保存，请等保存结束再打开来源。新材料产生的自动刷新通知会等本次保存结束后再处理，避免文件刚生成就中断登记；刷新后会保留保存结果提示。保存前仍会核对原始证据和 Wiki 内容，关闭会话、手动刷新或切换到新查询仍可能取消操作。若提示失败或操作被中断，先检查 `Sources` 和 `Activity`：看到材料文件不代表它已登记或排队，不要直接反复保存。

`Save to Wiki` 不会自动恢复已暂停的后台队列。暂停时，已登记材料仍需等待后续处理；它不会立即生成提案或修改 Wiki。

保存后 Studio 可能短暂刷新。刷新、重新加载以及加载失败时，已经确认的保存结果提示都会保留；同一知识库恢复可用后仍停留在原页签，不会因为刷新自动跳回 Activity。旧回答和引用权限会清除，不能再次保存旧回答。若新状态需要恢复处理或 Query 已不可用，界面仍会转到对应的处理页；切换知识库、关闭 Studio 或插件重载不会保留这条临时提示。
