...
Current checkpoint: Commit A/B/C/D/E/F/G foundations and the G.1 runtime adapter foundation are implemented; G.2a adds the review-only startup coordinator, G.2b adds durable Manifest plan/intent plus the exact atomic Manifest/apply-ledger adapter, G.2c adds end-to-end durable apply-authority proofs, G.2d upgrades Queue to v5 with explicit no-journal recovery, G.2e adds the pure layout-ready startup gate, G.2f adds shared-envelope conditional startup release, G.2g upgrades the outer Runtime to v3 with a durable source-observation hand-off, G.2h adds strict project-owned Bundle discovery plus the plugin-level fail-closed prerequisite barrier, and G.2i gives the existing Projects stack explicit hot-reload/cross-Vault lifecycle ownership. Review v2 derives final intent from the immutable compiler plan, transaction journal v3 retains that intent and full source identity, and runtime envelope v3 re-proves Queue/Review/high-watermark/allocator plus Manifest read-set/source-order authority before the first target observation, again inside prepared publication, before entering applying, on unfinished startup recovery, across every shared-envelope mutation while the transaction is active, and when publishing Manifest success with its ledger. Source capture now uses an idempotent `captureId`, opaque token, first-write-wins hash/pipeline binding, and atomic Queue consumption instead of treating an allocator scalar as payload authority. An accepted claim without a journal is classified from one atomic snapshot and requires explicit continue or abandon; continue repeats full authority proof, while abandon atomically proves that no journal, Queue commit marker, or source-input ledger exists before writing a Queue v5 tombstone. The startup gate now orders Queue recovery, journal/commit convergence, Review hand-off convergence, and a single-envelope batch classification guarded by the exact Review revision. It never resumes the Queue or performs no-journal decisions, and its `observed_clear` result is not claim authority. Conditional release consumes only that exact Runtime/Review/Queue observation and repeats all write-recovery plus accepted-terminal proofs in the same mutation that changes the Queue to running; generic Resume cannot clear or launder `startup_recovery`, and an unconsumed bound observation blocks release. Pending Queue commit markers remain bound to their ledger. Generic Manifest storage cannot bypass the success path, unsafe legacy or stale-Manifest authority fails closed before new Wiki file access, and shared-page hashes advance for every co-owner. The new outer plugin barrier waits for layout, keeps ordinary Projects independent from Runtime readiness, strictly validates all configured Windows boundaries, and uses a stable delegating port plus dynamic session so no sentinel Bundle can reach a controller. ProjectManager is now retired before a new Project state owner is installed; Project files, scans, pending-write guards, context caches and project-mode parser ownership bind to the exact App/Vault lifecycle, and unload ordering cannot reawaken an old autosave. The barrier intentionally stops at workflow-adapters-unavailable: the inner Gate/release pair, complete recovery/file/compiler adapters, Bundle selector, recovery UI, and worker remain unconnected. The real Windows watcher must also be connected to the new hand-off and recheck artifact hashes before execution. Remaining semantic dependencies and the Windows acceptance gate remain pending. Commit H follows only after G.2 safely enables the runtime workflow. Verification details are recorded in [`../TODO.md`](../TODO.md). Repository-wide Prettier still has only the pre-existing, untouched `src/LLMProviders/chatModelManager.ts` baseline. No source code from the audited external candidates has been copied; attribution status is recorded in [`../THIRD_PARTY_NOTICES.md`](../THIRD_PARTY_NOTICES.md).
...
- startup Review reconciliation、Manifest target read-set、exact `ApplyCommitManifestPort` ledger、commit-boundary Queue/Review/high-watermark 原子复证、no-journal classify/continue/abandon、生产 Project Bundle 配置源及外层 plugin Barrier 已完成；仍需 schema/link/source-artifact read-set、内层 startup Gate/release + Studio recovery adapter 组装、history compaction/size threshold 和 safe compare-and-delete，或继续保持 delete unavailable。
...
- [x] 实现 shared-envelope conditional release：Coordinator 只把 clear Gate 的 Runtime/Review/Queue revision 送回 Runtime；Runtime 在一次原子 transform 中重证 Vault-global transaction、所有 Bundle 的写恢复证据、当前 marker/claim/applying 状态与 accepted terminal classifications，成功只改变 exact startup control 和 Queue/Runtime revision。普通 `resume()`/`pause()` 与跨 `recovery_required`/`commit_pending_ack` 的两步 generic CAS 都不能清除或洗白单调恢复门；同门必须保留 current claim/job/marker，running/startup 中 accepted pending anchor 只有 exact accepted begin 才能消费，跨门必须匹配 current Review/claim/job/active committed journal 的 exact projection，历史 ledger 与 generic fabricated abandonment 都不构成替换当前恢复证据的 authority；watcher high-watermark 不得删除、回退、同 revision 改写，Runtime v3 后任何推进都必须原子消费 exact bound observation，pending bound 也阻止 release；marker finalization metadata 必须精确，无 durable receipt 的 release commit-then-throw 必须重跑 Gate。
...
- [x] 实现 exact App/Vault/adapter-owned `ObsidianVaultSourceWatcher` 独立 adapter：对已注册 immutable watch plan 同步捕获 path/authority/captureId，durable allocate 后才 `readBinary` exact bytes，以 locally revalidated raw SHA-256 + captured pipeline fingerprint 仅经 hand-off commit，并复证 allocation/settlement identity；旧 lifecycle/plan continuation 均 fail closed。rename/delete/folder 破坏事件同步使旧任务失权并 quarantine source，显式替换 plan 前同路径 ABA 不可重新摄入。配套 exact-byte reader 可在 parser/model 前按 job hash 复读并把同一 bytes 交给后续 parser。该 adapter 未接 `main.ts`，不启动 worker或开放 Studio。
- [ ] 从 durable Manifest/source identity 与真实 parser/provider/schema 配置构建 watch plan，并以插件级 workflow generation 组装 listener-before-crawl、allocated/bound recovery、跨重启 destructive-change authority/revalidation、全量 scan、Gate/release、executor/Compiler；当前不保存历史 artifact bytes。
...
- `observed_clear` 只用于恢复 UI 的乐观展示；conditional release Core 已在 shared envelope 内按 Runtime/Review/Queue revision、无 active journal/marker/apply claim/failed applying 与 accepted terminal evidence原子复证。现有外层 plugin Barrier 只证明 Runtime/Projects/config，仍必须在安装 ready delegate 和启动每个配置 Bundle worker 前运行内层 Gate + conditional release；不能使用 read-check 后普通 Queue resume，也不能把 empty/running 或保留的 user/rate-limit pause 当作已执行启动编排。
