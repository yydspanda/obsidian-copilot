...
- startup Review reconciliation、Manifest target read-set、exact `ApplyCommitManifestPort` ledger、commit-boundary Queue/Review/high-watermark 原子复证、no-journal classify/continue/abandon、生产 Project Bundle 配置源、外层 plugin Barrier/recovery-only Gate、observation startup Core、同代 workflow lease 与 observation-only production composer 已完成；仍需把该 composer 接入 plugin-owned workflow session，并补齐 schema/link/source-artifact read-set、fresh Gate/conditional release、worker、recovery actions、history compaction/size threshold 和 safe compare-and-delete，或继续保持 delete unavailable。
...
- [x] 实现 production-facing 只读 Manifest/schema/profile loader、exact-byte reader、UTF-8 byte-only parser 与 Runtime/Queue/preparation/model-call capability Core；G.2q-b 已用 preflight 同代 lease 把 Manifest/schema loader 与 exact reader 接入内部 observation-only composer，但 parser execution 与 worker 尚未接入插件。当前不保存历史 artifact bytes，legacy parser 会二次按路径读取，不能直接复用。
...
- [x] 以 preflight-minted secret-free workflow lease 和 production observation composer 组装同代 owners/parsers/Profile Source、Runtime Manifest/observation journal、exact App/Vault reader、startup-held enqueue-only Queue 与上述 observation Core；lease invalidation 同步关闭 live session，初次收敛后可重复 quiesce/reprove 并同步 assert health。该内部 composer 未接 `main.ts`，没有 Release、worker、model、Review 或 Wiki capability。
- [ ] 把 G.2q-b composer 纳入 plugin-owned 长生命周期 workflow session，随后执行 fresh per-Bundle Gate、conditional release、完整 artifact/projection validator、exact-byte executor/Compiler/Review hand-off 与 worker；fresh adapter 必须在外泄前直接交给预期 Compiler。
...
- `observed_clear` 只用于恢复 UI 的乐观展示；conditional release Core 已在 shared envelope 内按 Runtime/Review/Queue revision、无 active journal/marker/apply claim/failed applying 与 accepted terminal evidence原子复证。现有外层 plugin Barrier 已在 Runtime/Projects/config/preflight 后 owner-first 运行 recovery-only Gate，但仍不调用 conditional release；安装 ready delegate 和启动每个配置 Bundle worker 前，必须把现有 observation Core 纳入同代 workflow lifecycle、补齐 artifact/projection validator，再用 fresh Gate observation 执行 conditional release。不能使用 read-check 后普通 Queue resume，也不能把 empty/running 或保留的 user/rate-limit pause 当作已执行 release。
