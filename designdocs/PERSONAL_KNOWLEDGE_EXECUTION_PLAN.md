...
registered separately. The frozen merged V4 artifact completed the bounded Windows
regression tracked by `PK-H3-UPSTREAM-V4-WIN` on September 18; its evidence and
retained limitations are in the [completion checkpoint](./progress/acceptance/2026-09-18-unattended.md).
Subsequent canonical drift remains `OPS-UPSTREAM-SYNC` work. Delete and every auto-accept/auto-apply policy remain
unavailable.
...
- Windows plugin load 初始化 singleton state foundation。G.2 后续已实例化真实 ingest/compiler worker、Studio read delegate 与窄 command adapter；Activity Pause/Resume/Cancel/Retry、Review Reject、显式 reviewed create/update Accept/fresh Apply、受限 Recovery command、分离的 scoped grounded Query coordinator，以及 Save-to-Wiki writeback coordinator 已接入。Query 本身仍只读 current applied Wiki；Save 只能生成并注册 managed source，随后必须经真实 watcher/Queue/Compiler `query_writeback` 与显式 Review/transaction/Apply，不拥有直接 Wiki 写权。worker 本身仍无 Review-decision 或 Apply 权限；delete 未实例化。
...
- Windows Studio 只在唯一 Bundle 完成 startup recovery、observation reproof、conditional release、worker composition 和 adapter staging 后进入 live-ready；任一缺失或 multi-Bundle 无 selector 仍 unavailable。Live-ready 只授予显式声明的 Activity/Reject/reviewed create-update Apply capability、current-applied-Wiki grounded Query，以及只能从当前合格答案生成 managed capture 的 Save-to-Wiki capability；recovery-only startup 只授予当前 atomic observation 允许的 Continue/Abandon。Save 不直接写 Wiki，后续必须经真实 watcher/Queue/Compiler `query_writeback`、显式 Review 和已有 transaction/Apply。这些能力都不隐含 delete、auto-accept、auto-apply 或通用 rollback 权限。
...
- [x] H.1：Knowledge Studio 暂存 grounded excerpts；opaque citation ref 只允许打开当前查询签发且 hash-verified 的 Markdown line、heading occurrence 或 quote。在该 H.1 checkpoint，PDF page jump 保持 disabled；后续 H.3 已接通代码与自动化。
