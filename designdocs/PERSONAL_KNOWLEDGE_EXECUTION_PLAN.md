...
### Commit G.1 — Windows Runtime Adapter Foundation ← Historical foundation implemented; production gates opened by G.2x–H.2
...
- [x] 接入插件级零网络 production preflight：严格 Bundle owners、同代 Projects、已 hydrate settings、固定 compiler/parser/prompt/route profile 与 main-renderer native fetch 同步组成 generation-owned lease；该项最初建立 Compiler private route，H.2 又在同一 lease 内增加 prompt、schema、authority 与 lifecycle port 均独立的 grounded-answer route。不读取第二次 keychain，不 invoke 任一路由，不运行 Gate/Queue/watcher/model/Review/Wiki，settings/Projects 变化与 unload 同步 close。Profile/Transport/Adapter/Compiler error 使用私有 brand，固定 code policy 才能进入 Queue rate-limit/有界 retry/terminal 分类；Compiler 与投影后的 executor error 都绑定 exact Queue attempt signal，provider 不决定 retry delay/count，transport 仍无内部 retry。
...
- [x] 以 preflight-minted secret-free workflow lease 和 production observation composer 组装同代 owners/parsers/Profile Source、Runtime Manifest/observation journal、exact App/Vault reader、Queue、observation Core 与 released worker；lease invalidation 同步关闭 live session、非 applying request 与 H.2 answer request，初次收敛后可重复 quiesce/reprove 并同步 assert health。该 composer 已由 `main.ts` 在 recovery clear 后激活，并在 convergence 后接入 fresh Gate/conditional Release。Worker 拥有 compile-to-Review 能力，不拥有 Review decision、transaction preparation、Wiki mutation 或 Query capability；Activity/Reject、reviewed Apply、Recovery actions 与 H.2 read-only grounded Query 均由与 worker 分离的 generation-owned Studio boundaries 持有。
...
- [x] H.2 Core/evidence contract：Wiki chunk 只作不可引用 context；只有逐条复证的 exact Markdown source excerpt 获得 request-local evidence id。所有 answer claim 必须是 `source_fact` 或 `inference` 并引用本次 allowlist，证据不足使用独立状态且无自由 summary 字段。
