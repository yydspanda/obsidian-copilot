...
- startup Review reconciliation、Manifest target read-set、exact `ApplyCommitManifestPort` ledger、commit-boundary Queue/Review/high-watermark 原子复证、no-journal classify/continue/abandon、生产 Project Bundle 配置源、外层 plugin Barrier/recovery-only Gate、observation startup Core、同代 workflow lease、observation-only production composer 与 plugin-owned observation session 已完成；仍需补齐 schema/link/source-artifact read-set、fresh Gate/conditional release、worker、recovery actions、history compaction/size threshold 和 safe compare-and-delete，或继续保持 delete unavailable。
