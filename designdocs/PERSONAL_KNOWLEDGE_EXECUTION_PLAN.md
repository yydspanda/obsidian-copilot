...
G.2t closes the source-preparation hand-off. The observation composer retains the authenticated execution plan produced for the watcher and creates a Runtime proof port from the exact same QueueStorage/execution owner. Its preparation executor accepts only an authentic Queue claim, obtains a Runtime proof, re-proves Manifest/schema/profile authority, reads and hashes exact source bytes, routes those bytes through the plan-owned byte parser, then exposes only an authentic `KnowledgeAuthorizedSourcePreparation` to a constructor-captured handler method. Structurally similar plan/proof objects, handler accessors, signal substitution, or stale generations fail closed. Production Compiler target/projection adapters, DeepSeek route retention, Review hand-off, and `main.ts` worker creation remain pending.
