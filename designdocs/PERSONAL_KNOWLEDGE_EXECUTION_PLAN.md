...
- [x] 实现 production-facing 只读 Manifest/schema/profile loader、exact-byte reader、UTF-8 byte-only parser 与 Runtime/Queue/preparation/model-call capability Core；G.2o 已在插件零网络 preflight 中实例化 Profile Source 和 owned parser registry 以验证 identity/route，但 Manifest/schema loader、exact reader、parser execution 与 worker 尚未接入。当前不保存历史 artifact bytes，legacy parser 会二次按路径读取，不能直接复用。
...
- [ ] 以插件级 workflow generation 继续组装 listener-before-crawl、allocated/bound recovery、跨重启 destructive-change authority/revalidation、全量 scan、Gate/release、exact-byte executor/Compiler/Review hand-off；fresh adapter 必须在外泄前直接交给预期 Compiler。
