...
G.2t closes the source-preparation hand-off. The observation composer retains the authenticated execution plan produced for the watcher and creates a Runtime proof port from the exact same QueueStorage/execution owner. Its preparation executor accepts only an authentic Queue claim, obtains a Runtime proof, re-proves Manifest/schema/profile authority, reads and hashes exact source bytes, routes those bytes through the plan-owned byte parser, then exposes only an authentic `KnowledgeAuthorizedSourcePreparation` to a constructor-captured handler method. Structurally similar plan/proof objects, handler accessors, signal substitution, or stale generations fail closed. This bridge itself owns no model, target, Review, or Wiki mutation capability; G.2u supplies the first two production capabilities while the remaining handler and `main.ts` worker composition stay pending.

G.2u gives model and Vault-read capabilities explicit production ownership. The zero-network preflight now retains each validated DeepSeek private route in a Bundle-keyed opaque lease whose lifecycle owner is held only by the preflight generation. The admitted lease has no close, route, settings, credential, or adapter getter; its compile boundary creates the job-bound model adapter and hands it directly to a new Compiler before awaiting or returning. Invalidation clears unborrowed route closures before workflow revocation. The renderer fetch function and receiver are captured together, so later mutation of `window.fetch` cannot intercept authorization headers. Separately, `ObsidianKnowledgeCompilerTargetResolver` captures the exact App/Vault/adapter and uses a Windows-keyed loaded-file snapshot: create-only targets perform existence checks without content reads, while authorized targets use canonical Vault spelling and exact adapter reads with stat/index/owner reproof. Collisions, drift, malformed payloads, and cancellation fail closed. These capabilities are not invoked by `main.ts` yet; a branded attempt/input builder must bind the authentic preparation, exact Queue claim/reporter, and derived input before the lease can be used. Semantic candidate validation, Review hand-off, failure/no-change projection, Queue-owned active-request cancellation/durable unwind, and worker scheduling also remain pending; an alternate transport signal must not replace the exact Queue signal.
...
- [ ] 为 preparation handler 组装 production target/projection validator、同代 DeepSeek route、Compiler 与 Review hand-off，再由 `main.ts` 创建 bounded worker；fresh model adapter 必须在外泄前直接交给预期 Compiler。
