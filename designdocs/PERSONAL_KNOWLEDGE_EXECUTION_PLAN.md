...
- startup Review reconciliation、Manifest target read-set、exact `ApplyCommitManifestPort` ledger、commit-boundary Queue/Review/high-watermark 原子复证、no-journal classify/continue/abandon、生产 Project Bundle 配置源、外层 plugin Barrier 与 recovery-only Gate 组装已完成；仍需 schema/link/source-artifact read-set、allocated/bound observation recovery、conditional release、recovery actions、history compaction/size threshold 和 safe compare-and-delete，或继续保持 delete unavailable。
...
- [ ] 以插件级 workflow generation 继续组装 listener-before-crawl、allocated/bound observation recovery、跨重启 destructive-change authority/revalidation、全量 scan、完整 artifact/projection validator、conditional release、exact-byte executor/Compiler/Review hand-off 与 worker；fresh adapter 必须在外泄前直接交给预期 Compiler。
