...
- startup Review reconciliation、Manifest target read-set、exact `ApplyCommitManifestPort` ledger、commit-boundary Queue/Review/high-watermark 原子复证、no-journal classify/continue/abandon、生产 Project Bundle 配置源、外层 plugin Barrier/recovery-only Gate 与 observation startup Core 已完成；仍需把 observation Core 纳入 production workflow generation，并补齐 schema/link/source-artifact read-set、conditional release、recovery actions、history compaction/size threshold 和 safe compare-and-delete，或继续保持 delete unavailable。
...
- [ ] 以插件级长生命周期 workflow generation 组装同代 Manifest/schema/profile capability、startup hold、上述 observation Core、fresh per-Bundle Gate、conditional release、完整 artifact/projection validator、exact-byte executor/Compiler/Review hand-off 与 worker；fresh adapter 必须在外泄前直接交给预期 Compiler。
