...
- [ ] 实现 production Manifest/schema/profile loader 与 byte-only parser/executor，并以插件级 workflow generation 组装 listener-before-crawl、allocated/bound recovery、跨重启 destructive-change authority/revalidation、全量 scan、Gate/release、executor/Compiler；当前不保存历史 artifact bytes，legacy parser 会二次按路径读取，不能直接复用。
