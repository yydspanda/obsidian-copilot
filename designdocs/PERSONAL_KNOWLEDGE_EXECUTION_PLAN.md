...
- [x] 实现 production-facing 只读 Manifest/schema/profile loader、exact-byte reader、UTF-8 byte-only parser 与 Runtime/Queue/preparation/model-call capability Core；这些端口尚未在插件中实例化。当前不保存历史 artifact bytes，legacy parser 会二次按路径读取，不能直接复用。
...
- [ ] 在 production 接线前单独迁移插件 DeepSeek model catalog 与已有 Project 配置，从已退役的 `deepseek-chat` / `deepseek-reasoner` 转向当前 V4 identity；thinking profile 还须从当前默认 `temperature=0.1` 显式迁移为 Knowledge-effective `temperature=0` 且不配置 `topP`，或显式关闭 thinking 后再使用采样参数，并清理/拒绝其余未映射 generic provider 字段。不得在 Knowledge transport 内用 alias、fallback 或被 provider 静默忽略的参数隐藏配置变化，并须独立回归普通 Chat。
- [ ] 以插件级 workflow generation 组装 settings/key/native-fetch lifecycle、listener-before-crawl、allocated/bound recovery、跨重启 destructive-change authority/revalidation、全量 scan、Gate/release、exact-byte executor/Compiler/Review hand-off；fresh adapter 必须在外泄前直接交给预期 Compiler。真实 worker 接线前定义 sanitized provider error → Queue rate-limit/infrastructure mapping，transport 本身仍不得 retry。
