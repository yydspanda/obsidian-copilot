...
interface IngestQueueSnapshotV4 {
  version: 4;
  bundleId: string;
  revision: number;
...
`pipelineFingerprint` 覆盖 contract、完整 Bundle config digest、exact schema hash、compiler version/configuration、selected parser identity/configuration、调用方投影后的 model behavior、输出语言、OKF 与 citation contract，不能只凭 source hash 判断是否跳过，也不能包含 API key 或完整 provider settings。调用方必须先把真实设置投影为显式 allowlist；Core 对常见 credential-like key 的拒绝只是纵深防御，不替代 production adapter 的 allowlist。路径覆盖、越界写入、同文件并发修改和部分失败必须在应用 ChangeSet 前由确定性代码处理。模型只能提出 ChangeSet，不能自己宣称事务成功。

G.2k 将 watcher 的 fingerprint 来源收紧为 opaque `KnowledgeSourceWatchPlan`：builder 对每个 Bundle 的 detached Bundle/Manifest snapshot 重新 strict 解析并验证关系，统一检查 durable source 与 sourceRoots/Wiki/schema 及历史 generated-page 写边界，以 exact schema bytes 计算 raw hash，并从 caller-projected compiler/parser/model/output-language/OKF/citation behavior profile 中为每个 source 无歧义选择一个通用 suffix parser。per-source fingerprint 绑定完整 Bundle-config digest、compiler version/configuration、selected parser identity/configuration、schema hash 与其余投影行为；plan 另保留 Manifest revision/digest、submitted-profile digest 和总 digest。输出不保留 schema bytes 或配置值。watcher 的构造和替换入口不再接受 raw source array；module-private constructor token、WeakMap state 以及 frozen class/prototype/instance 共同拒绝 direct construction、内部容器反射与 method monkeypatch。相同物理 source 可以显式服务不同 Bundle，但同 Bundle 的 Windows source identity 冲突、无 parser/多 parser、schema 自摄入和越界 source 全部 fail closed。这个 Core 仅防御性拒绝常见 credential-like field；production adapter 仍须显式 allowlist，并证明提交快照对应当前 durable Manifest/schema/settings。G.2l 已实现只读 loader/parser Core；production `main.ts`、exact-byte executor/Compiler composition 与 worker 仍未接入。
...
首期 runtime foundation 把 Queue、Review、Manifest、Vault-global active transaction、watcher observation journal 和 apply ledger 放进 `knowledge-runtime-v1.json`。文件名保持稳定，strict outer envelope 已升级为 v3。v1 仍只有在没有 active transaction、apply/review recovery state、历史 Manifest success 或 reserved commit metadata 等无法补证的状态时才原子迁移，其余状态 typed fail closed 且保留原字节；v2→v3 可保留合法 active/recovery/ledger，但必须证明现有 Queue high-watermark 不超过 scalar allocator。迁移只把既有 revision 设为 legacy fence，并保存当时 exact watermark checkpoint，不为历史读取伪造 token。v3 每次读取继续交叉验证 Manifest success、reserved metadata、该 source 最新 ledger、Manifest revision/digest 与共享页 co-owner projection，并新增 observation journal↔Queue watermark 校验。每次 mutation 通过一个同步 transform 全量解析、校验所有 slot、提升 envelope revision、序列化并替换，从而让不同 facade 共享同一 CAS 边界。source watcher 必须在任何异步读文件之前生成稳定 `captureId` 并幂等取得 opaque token/`inputRevision`；读完后 token 以 first-write-wins 绑定 exact source hash/pipeline。Queue high-watermark 只有匹配 bound record 才能推进，Queue snapshot 与 bound→consumed 在同一 transform 发布；`settle` 处理无 Queue 写入的 dedup 和 commit-then-throw。更高的未绑定 allocation 不会丢弃较低 bound read，更高 bound read 才会 supersede 较低未终结观察；任一 pending allocated/bound record 都阻止 startup release。首次文件初始化设计为使用同目录完整临时文件、handle flush 与排他 hard-link 发布；自动化竞争测试验证不会暴露 winning initializer 的空或半 JSON，真实 Windows 行为仍由后述验收门槛负责。
...
成功账本采用可重试的交接协议：committed journal 先与 Queue 中完整 source/hash/pipeline/input revision/job attempt claim 做只读精确核验；通过后，shared envelope 的同一次 transform 再交叉验证 exact Queue claim、Review accepted record 的 proposal/accepted/intent identity，以及 `allocator ≥ source high-watermark ≥ apply claim`，然后才更新完整 Manifest projection 与 append-only apply ledger。较新的同内容观察可以与旧 apply 并存；较新的不同内容必须由 retained rerun 或启动恢复后 promoted successor 精确证明，而 retained rerun 本身必须始终等于最新 high-watermark。随后系统再把 Queue job 与 `commit_pending_ack` marker 原子落盘，清除全局 journal，最后移除 Queue marker 并保留 `startup_recovery` 暂停；每次 runtime parse 都把尚未清除的 marker 与同 transaction ledger 的 source/hash/pipeline/input/ChangeSet/digest/revision/time 精确交叉验证，因此 active journal 已清除也不能靠孤立 marker 冒充成功。ledger exact replay 在任何 Queue/Review/active-journal/Manifest 检查前就是 byte-preserving no-op，同 transaction id 的不同 identity fail closed；已入 ledger 的 transaction id 也不能重新发布 prepared journal。新 apply 会先用无文件访问的 authority port 复证 Queue/Review/high-watermark/allocator、Manifest intent read-set 与 source commit ordering，之后才允许 validator 观察 target；首次 prepared 发布在 shared-envelope transform 内重复证明，关闭预检后的竞争窗口。prepared → applying、启动读取 prepared/applying journal，以及活动事务期间每次 Queue/Review/allocator/transaction-progress mutation 都必须保留这套完整 reservation，确保 Queue v3 缺失最终 intent digest 或 Manifest 已漂移的事务在任何新 Wiki 文件观察或 mutation 前停止；活动 journal 存在时，普通 Manifest CAS 被锁定，通用 Manifest storage 也不能创建或修改 `lastSuccessful` 与 runtime reserved metadata，从而使 `recordApplyCommit` 成为唯一成功账本路径。shared page update 会要求所有实际 owner 的 path/ownership/hash 一致，并在同一 Manifest transform 中传播新 hash，避免 co-owner projection 撕裂。当前 success ledger 只接受有 durable Review authorization 的 claim；`multi_file` / `trusted_generated_only` 的自动应用必须先增加独立持久 policy authorization，不能仅凭内存判断绕过审核。已有 journal、ledger 或 Queue marker 的断点可以按各自 exact identity 重试；accepted claim 尚未创建 journal 时，Runtime 从同一 atomic snapshot 区分 accepted-not-started、active、blocked、finalizing、committed、abandoned 与真正 no-journal candidate。真正 candidate 只接受显式 continue 或 abandon：continue 重新加载 durable accepted payload 并重证当前 Manifest authority，随后 prepared publication 再原子重证；abandon 在同一 envelope transform 中证明任何 phase 的 active journal、Queue commit marker 与同 source/input ledger 均不存在，保留 accepted Review，并把 cancelled job 与 exact Queue v5 tombstone 一起提交。两者竞争时只能有一方成功，commit-then-throw 后可从 durable state 重放收敛；系统仍不会自动继续。Manifest adapter、Queue adapter 和 journal adapter 都必须提供各自契约要求的 durable CAS，普通的 read-then-write 不满足要求。
...
G.2e 将已有 Core 组装为 layout-ready startup gate，但仍不直接接 Obsidian 生命周期。Gate 必须接收严格验证的当前 Bundle 配置，先执行 Queue startup recovery，再让 `ApplyCommitCoordinator` 收敛已有 active journal/commit marker，之后收敛 Review pending/rejected hand-off；最后 Runtime 在一次 shared-envelope read 中核对 exact Review revision，同时返回 Queue snapshot、runtime revision、Vault-global transaction slot 与全部 accepted classifications。prepared/applying journal 的 roll-forward 可能真实观察并 CAS Wiki，因此不能用 journal 内旧 Bundle 或默认 bundle id 冒充当前配置；首次发现第三态会先持久化 sticky `recovery_required`，Gate 只针对该 typed failure 立即重读一次，而且第二次必须是同 transaction、同 Bundle 的 blocked 结果。Review revision 在收敛与单-envelope 分类之间变化时整轮有界重跑；跨轮 Runtime revision、初始/ApplyCommit/最终 Queue revision 都只能单调前进，同 Queue revision 必须表示完全相同的规范化内容。Review 输出的每条 accepted identity 与 Runtime classification 还必须通过共享的 content-addressed recovery id 构成无重复、顺序无关的一一对应集合，缺项、多项或错配全部 fail closed。最终 atomic snapshot 中后来出现的 Vault-global transaction 即使与当前 Bundle 没有 accepted record，也会形成 blocker。Gate 不拥有 begin/continue/abandon/model/resume 能力；`accepted_not_started` 与真正 no-journal candidate 都保持显式 attention。`observed_clear` 只是带 runtime/review revision 的乐观展示结果，不是新 claim 的授权。

G.2f 增加独立 conditional startup release Core。Coordinator 只接受 `observed_clear`，并把 Bundle id 与 Gate 观察到的 Runtime、Review、Queue revision 组成窄令牌；Runtime 在单次 shared-envelope transform 中重证这三层 revision、Vault-global transaction 为空、其他 Bundle 不存在 apply 写恢复证据、当前 Queue 没有 commit marker/apply claim/failed-or-processing applying/paused job，并重新分类全部 Bundle 的 accepted Review，只有 `committed` 或 `abandoned` 才是 terminal。G.2g 先把 bound-but-unconsumed source observation 纳入 release blocker，G.2q-a 再补齐 allocated observation，防止启动越过任何已持久化但尚未入队的捕获阶段。目标 Queue 必须精确处于 `paused/startup_recovery`；成功只将 control 改为 running，同时各提升一次 Queue 与 outer revision。任何 observation 变化、user/rate-limit pause 或恢复 blocker 都不写字节。普通 `resume()` 不能清除 startup pause，`pause()` 也不能把它洗成 user pause；generic Runtime Queue CAS 把 `startup_recovery`、`recovery_required` 与 `commit_pending_ack` 全部作为单调恢复门。同门 mutation 必须原样保留 current claim/job/marker；accepted-not-started pending anchor 在 running 与 startup gate 下都不能被 generic CAS 擦除，只有 matching accepted Review 的 exact begin 投影可把它变为 applying claim。watcher 可在不改变 claim/review authority 的前提下修改 processing/awaiting job 的 rerun flag 和单调 `updatedAt`，但 source high-watermark 不可删除、回退、在同 revision 改写；v3 后任何推进还必须原子消费 exact bound observation，而不再只检查 allocator 上界。startup → recovery 必须等于当前 processing apply 的 exact recovery 投影；startup/recovery → commit-pending 必须同时绑定当前 claim/job、当前 active committed journal 及 ledger，并完全等于 Queue resolution 投影；journal 已清除后的 marker → startup 必须重建固定 detail、非回退时间与其余完全相同的 Queue payload。`applyAbandonments` 对 generic Queue writer 始终只读，只有 Runtime 专用 abandon transform 能在同一 authority reproof 中追加。这样历史 ledger 和伪造 abandonment 都不能替换当前 interrupted/accepted-not-started apply，任何中间门也不能通过后续 generic CAS 降级为 running/user/rate-limit。`accepted_not_started` 不通过 release，但显式 exact acceptance 可以在 running 或保持 startup pause 时建立 apply claim，之后仍走完整 Runtime apply authority；若 claim 后 journal 前崩溃，下一次 startup 会升级到 no-journal recovery。release 没有 durable receipt，因此 commit-then-throw 后不会用 `revision + 1` 猜成功：旧令牌返回 observation changed，调用者必须重跑 Gate，fresh observation 会看到 already running。每个 Bundle 独立 Gate/release，但共享 journal 导致跨 Bundle apply recovery 全局 quarantine；生产 singleton 仍必须在暴露 worker/Resume 前完成所有配置 Bundle 的启动编排。

G.2q-a 把 observation 启动收敛做成不持有 Queue control 的独立 Core。`ObsidianVaultSourceWatcher.startListening()` 先同步注册 create/modify/delete/rename，再由 reconciler 处理旧 journal，之后才显式 full crawl；`start()` 在 crawl 激活后保持幂等，listener-only 阶段替换 plan 不会偷偷扫描。该 crawl 只覆盖 opaque Manifest-derived plan 中已经注册的 source，不遍历 `sourceRoots` 自动发现新文件。启动缺失、exact path 大小写漂移、Windows-key collision、capture failure 与 rename/delete 都进入当前 plan generation 的 authoritative blocker，并 quarantine 对应 source；同代 create 不能把旧 destructive/missing 证据洗白，只有新 plan generation 可重新证明。重启 full crawl 只能复证启动后的当前状态，不持久保存停机期间曾发生 rename/delete/recreate 的事件证据。reconciler 对 allocated token 不绑定停机期间猜测的字节；bound observation 也必须先以 `readExpected(path, oldHash)` 读取并本地复算相同 raw hash、匹配当前 plan/pipeline 后才原样重放 token/hash/pipeline。allocated 或 hash/path drift 只有在新 crawl capture 成功时才由更高 revision supersede，残留记录在 final reconciliation 继续阻塞。协调器固定执行 listener → pre-crawl recovery → crawl → idle → final recovery → idle → blocker proof，并在调用方 generation 取消时同步关闭 watcher/reconciler。它的成功结果只表示 observation 已收敛，不是 Gate receipt、Queue release receipt 或 worker authority；现已由同代 plugin startup observation session 在 recovery clear 后激活。

G.2q-b 把上述 Core 放进仍然 observation-only 的内部 production composition。零网络 preflight 现在从同一次 Projects/settings/resources snapshot 创建并复用 exact `ProjectKnowledgePipelineProfileSource`，再签发 module-authentic、secret-free workflow lease；lease 只持有 frozen Bundle owners、owned parser capabilities、profile resolve authority 与 current/invalidation 边界，不持有 API key、transport 或 settings。替换、Settings/Projects 变化或 close 会先清除 capability 引用，再从稳定 listener snapshot 同步通知失效。`KnowledgeProductionObservationComposer` 用该 lease 和同一个 execution owner 组装 Runtime-backed Manifest repository、observation allocator/binder、exact App/Vault schema/source reader、double-collected workflow plan 与 startup-held Queue。只有 narrow `enqueue` 进入 `SourceObservationHandoff`；Queue 的 private executor 对任何意外 claim/run fail closed，因此 observation 不能升级为 worker。初次收敛保留 live watcher；同一个 caller signal 可串行调用 `reprove()`，先等待当前 capture idle、重放可证明的 residual observation、再次 idle 并检查 blocker，`assertHealthy()` 则同步证明当前会话仍无 blocker且未被撤销。stale/mismatched signal、并发 reproof、健康检查失败或 lease invalidation 都会关闭整个 session，不生成可转移的 Release token。G.2q-c 已将该 composer 由 `main.ts` 在 recovery clear 后接入同代 plugin-owned session。

G.2q-c 把 production observation session 接到 plugin-owned startup 生命周期：Barrier 严格验证 observation port，只在同代 recovery 返回 `observed_clear` 后启动，并保留 live watcher；新 generation、取消、Settings/Projects/Runtime replacement 与 unload 都同步 close。只有完整 `observation_converged` 结果才算收敛，blocker/diagnostic 会 fail closed，绝不伪装成 Release receipt。

G.2r 在同一 session 上完成 fresh Gate/conditional Release：初次 convergence 后先调用同 signal 的 `reprove()` 并同步 `assertHealthy()`，再按 Bundle 顺序运行 fresh Gate；每个 clear 结果只通过 Runtime shared-envelope release port 重新证明 revision、Queue startup pause、跨 Bundle recovery、accepted terminal classification 与 source-observation watermark。任一 stale/blocked/observation drift/代际替换都关闭 session，成功只表示 Queue startup control 已被 Runtime 原子改为 running，不提供 claim、worker、model、Review decision、transaction preparation 或 Wiki mutation capability。

G.2s/G.2t 增加 bounded worker 与 exact preparation bridge。Worker 每次显式 `runOnce()` 最多按稳定 Bundle 顺序 claim 一项，并要求同代 release/health proof；observation composer 保留 watcher 使用的 authenticated execution plan，并用同一个 Runtime QueueStorage 创建 proof port。真实 executor 必须依次取得 Queue claim、Runtime proof、Manifest/schema/profile reproof、exact bytes 与 plan-owned byte-parser 结果，最后只把 authentic `KnowledgeAuthorizedSourcePreparation` 交给构造期固定捕获的 handler。该桥自身不持有 target、model、Review 或 Wiki mutation capability；后续 capability 必须按同一代次显式注入，在完整 handler 就绪前 `main.ts` 不创建 worker，也不会发起模型请求或 Wiki mutation。

G.2u 把模型 route 与 Vault target read 从“已实现但未持有”推进为可组装的生产 capability。零网络 preflight 按同一 Settings/Projects snapshot 创建每个 Bundle 的 DeepSeek private route，并将其保存在独立、可同步撤销且不暴露 `close` 的 opaque lease；credential、settings、route 和 model adapter 均不进入 admission 的可枚举数据，lease 只在 authentic preparation 上内部构造 job-bound adapter，并在任何外部可观察点之前立即交给新的 Compiler。Settings/Projects/replacement/unload 先清空尚未借出的 route closure，再撤销 secret-free workflow lease；renderer fetch 同时固定捕获 exact function 与 receiver，避免后续替换 `window.fetch` 截获 Authorization header。首个 `ObsidianKnowledgeCompilerTargetResolver` 则绑定 exact App/Vault/adapter，以 Windows comparison key 查 canonical loaded path：`create_only` 已占用时不读内容，authorized target 才做 exact adapter read，并在 stat/index/owner 前后复证。大小写碰撞、目录/文件漂移、malformed payload、accessor 与取消全部 fail closed。当前仍缺 branded compile-attempt/input builder、共享 OKF/link/citation candidate validator、Review hand-off、failure/no-change durable projection 和 worker scheduler；builder 必须把 preparation、同一 Queue claim/reporter 与派生 input 绑定，Queue lifecycle 也必须在启用 worker 前用自身 active controller 中止 pending exact signal/native fetch。不能以另一个 transport signal 取代既有 exact Queue-signal 契约。因此这些 capability 尚未被 `main.ts` 调用。

G.2b/G.2c 已关闭“只凭 changed targets 猜 Manifest”和“旧审核事务先写文件、最终提交才发现授权不足”的缺口：Compiler 输出 versioned Manifest commit plan，Review v2 保存包含 target authorization 的 plan 并投影 final intent，transaction journal v3 的 final intent 通过 plan digest 回链该授权，同时保存完整 post-compile page projection、显式 ownership、Manifest revision/digest read-set 与完整 source identity。Runtime 在直接 apply 的第一次 target observation 前、prepared 发布、进入 applying、启动恢复读取和活动事务的每次 envelope mutation 时复证或保留该完整 read-set/authority，页面 committed 后再把 Manifest success 与 exact ledger 原子发布；顺序调用 Repository 再 append ledger 被明确禁止，单调 `inputRevision` 也不再由 `completedAt` 猜测。该 core 仍未接入 plugin startup/Studio，不等于 Golden Flow 已可用。

在接入真实 Vault 写盘前还必须关闭这些边界：把 schema、非 target link 和 source artifact 的 hash/read-set journal 化并在恢复前复证；在已接入的 plugin-owned observation session 上完成每个 Release 决策前的 fresh observation reproof；compiler 消费 artifact 时必须调用 exact reader 重新核对 job hash，并把返回的同一 `Uint8Array` 直接交给新的 byte-only parser，不能复用会二次按路径读取或走缓存的 legacy `FileParserManager`；明确 CAS 已完成但 progress 尚未落盘时的 content ABA 策略；为已实现的 startup gate/release 提供完整 validator/file/compiler adapter composition、插件级 singleton/startup barrier/delegating Studio port 与恢复 UI；定义无文件 ChangeSet 的 durable source-success/Manifest revision 语义，不能把 `no_changes` 静默当作已摄入；为自动应用模式增加不可伪造的 durable policy authorization。Observation 与 Queue/Review 终态历史还没有压缩，删除前必须设计等价 tombstone，不能破坏 capture replay、allocator floor 或旧输入不可复活保证。极端 outer-envelope revision 接近 `MAX_SAFE_INTEGER` 时，还需为整条 transaction/manifest/queue/ack 序列预留 revision 容量。当前纯 core 采用 content-addressed at-least-once 判定，若用户在停机期间把文件精确恢复为 before bytes，恢复无法区分“尚未写入”与“用户撤销”；个人知识资产默认应优先考虑 mutation-intent marker 加 fail-closed，代价是更频繁的人工恢复。conditional release 已关闭 clear Gate 后普通 `resume()` 的 TOCTOU，但 empty/running Queue 和保留的 user/rate-limit pause 本身不编码 startup epoch；生产 singleton 必须在 observation 收敛、fresh Gate 与 conditional release 之间持续保持同一健康 generation，完成每个配置 Bundle 的 release 或明确 blocker 后，才能允许 scheduler 与 Resume 入口工作。
...
- Phase 2 可选择性监听并发现用户授权的 `sourceRoots`；首期 `sourceRoots` 只是授权边界，crawl 只处理 Manifest 已注册来源。Project 切换时 flush/suspend，防止旧状态覆盖新项目。
