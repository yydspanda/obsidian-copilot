...
在接入真实 Vault 写盘前还必须关闭这些边界：把 schema、非 target link 和 source artifact 的 hash/read-set journal 化并在恢复前复证；为生产 source watcher 增加专用 observation hand-off，把 allocator revision 与随后读取到的 hash/pipeline 持久绑定，而不是把 scalar allocator 当作 payload authenticity proof；明确 CAS 已完成但 progress 尚未落盘时的 content ABA 策略；为已实现的 startup gate/release 提供生产 Bundle 配置源、完整 validator/file adapter composition、插件级 singleton/startup barrier/delegating Studio port 与恢复 UI；定义无文件 ChangeSet 的 durable source-success/Manifest revision 语义，不能把 `no_changes` 静默当作已摄入；为自动应用模式增加不可伪造的 durable policy authorization。极端 outer-envelope revision 接近 `MAX_SAFE_INTEGER` 时，还需为整条 transaction/manifest/queue/ack 序列预留 revision 容量。当前纯 core 采用 content-addressed at-least-once 判定，若用户在停机期间把文件精确恢复为 before bytes，恢复无法区分“尚未写入”与“用户撤销”；个人知识资产默认应优先考虑 mutation-intent marker 加 fail-closed，代价是更频繁的人工恢复。conditional release 已关闭 clear Gate 后普通 `resume()` 的 TOCTOU，但 empty/running Queue 和保留的 user/rate-limit pause 本身不编码 startup epoch；生产 singleton 必须先完成每个配置 Bundle 的 Gate/release 或明确 blocker，再允许 scheduler 与 Resume 入口工作。
