...
在接入真实 Vault 写盘前还必须关闭这些边界：把 schema、非 target link 和 source artifact 的 hash/read-set journal 化并在恢复前复证；明确 CAS 已完成但 progress 尚未落盘时的 content ABA 策略；为已实现的 startup gate 提供生产 Bundle 配置源、完整 validator/file adapter composition、插件级 singleton/delegating Studio port 与恢复 UI；定义无文件 ChangeSet 的 durable source-success/Manifest revision 语义，不能把 `no_changes` 静默当作已摄入；为自动应用模式增加不可伪造的 durable policy authorization。极端 outer-envelope revision 接近 `MAX_SAFE_INTEGER` 时，还需为整条 transaction/manifest/queue/ack 序列预留 revision 容量。当前纯 core 采用 content-addressed at-least-once 判定，若用户在停机期间把文件精确恢复为 before bytes，恢复无法区分“尚未写入”与“用户撤销”；个人知识资产默认应优先考虑 mutation-intent marker 加 fail-closed，代价是更频繁的人工恢复。即使 Gate 观察为 clear，重新 claim 前仍需在 shared envelope 内把 runtime/review revision、无 active journal/marker/apply claim/failed applying 与 accepted terminal evidence 同原子 conditional release 复证，不能以普通 `resume()` 接在 read-check 后面。
